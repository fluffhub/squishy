...
M.Import('squishy/graphics.js',function(gfx) {
  ...
  M.Class(function C() {
    C.Super(LayoutItem);
      ...
      startVideo:function() {
        if (navigator.getUserMedia) {
          navigator.getUserMedia({video: true}, this.get_handler(this.handleVideo), this.videoError);
      ...
      handleVideo:function(stream) {
        ...
        this.lms=stream;
        this.video.onloaddeddata=this.get_handler(this.scaleVideo);
      },
      scaleVideo:function() {
    ...
    C.Init(function ImageGetter(callback) {
      ...
      this.trigger=new MomentaryButton(' ','icon-camera trigger',this.get_handler(this.shoot));

      this.camera=new MomentaryButton(' ','icon-camera webcam',this.get_handler(this.startVideo));

      this.uploadform=new Form('uploader');
      this.uploadfield=new ImageFileInput('imgdata','',this.get_handler(this.getFile));
      this.uploadbutton=new MomentaryButton(' ','icon-upload ',this.get_handler(this.clickupload));
      ...
      this.imageReader.onloadend=this.get_handler(this.handleImageFile);
      ...
      this.tempImg.onload=this.get_handler(this.onchange,this.tempImg);
      this.resize();
    });
  });
});
