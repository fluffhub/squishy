  ...
  M.Import('squishy/DOM','squishy/events','squishy/styles','squishy/svg',function(DOM,events,styles,svg) {
    ...
    var Draggable=M.Class(function C() {
      ...
      C.Def(function enabledrag(callback, target) {
          ...
          this.enableEvents('dragstart','drag');
    ...
    M.Def("defaultResizeFunctions", {
      e: function e(re,handle) {
        re.size.width=re.size.width+handle.delta.x;
      },
      w: function w(re, handle) {
        re.position.x=re.position.x+handle.delta.x;
        re.size.width=re.size.width-handle.delta.x;
      },
      s: function s(re, handle) {
        re.size.height=re.size.height+handle.delta.y;
      },
      n: function n (re, handle) {
        re.size.height=re.size.height-handle.delta.y;
        re.position.y=re.position.y+handle.delta.y;
      },
      nw: function nw(re,handle) {
        re.position.y=re.position.y+handle.delta.y;
        re.position.x=re.position.x+handle.delta.x;
        re.size.width=re.size.width-handle.delta.x;
        re.size.height=re.size.height-handle.delta.y;
      ...
      se:function se(re,handle) {
        //change width in X
        //change height in Y
        re.size.width=re.size.width+handle.delta.x;
        re.size.height=re.size.height+handle.delta.y;
      },
      sw:function sw(re,handle) {
        //change left in X
        //change height in Y
    ...
    var Resizable=M.Class(function C() {
      ...
      C.Def(function enableresize(dirs,callback, handles) {
          if(!this.handles) {
            Object.defineProperty(this, "handles", {value:{}, writable:true});
          }
          if(!this.resizers) {
            Object.defineProperty(this, "resizers", {value:extend({}, M.Self.defaultResizeFunctions), writable:true});
          }
          if(handles instanceof Object) {
            extend(this.resizers, handles);
          }
          if(!dirs) dirs="nw,ne,se,sw";
          dirs=dirs.split(",");
          this.dirs=dirs;
          ...
          var stophandler = this.resizestophandler()
          var starthandler = this.resizestarthandler();
          var resizehandler = this.resizehandler();
          for (var i in dirs) {
            dir=dirs[i];
            this.handles[dir]=new ResizeHandle(dir);
            this.handles[dir].ondragstart=starthandler;
            this.handles[dir].ondragend=stophandler;
            this.handles[dir].onmove=resizehandler;
            this.add(this.handles[dir]);  
            this.handles[dir].show();
          }
          if(callback instanceof Function) {
        ...
        C.Def(function doResize(handle, skip_handler) {
          (this.resizers)[handle.dir](this,handle);
          this.drawTransform();
          if(!skip_handler)
            this.onresize(handle);
        });

        C.Def(function resizestarthandler() {
          var that=this;
          return function(s) {
            this.enableEvents("dragend");
            if(that.onresizestart instanceof Function) {
              that.onresizestart(this, s);
            }
              console.debug({"resizing":this.dir})
      
            
           }
        });
        C.Def(function resizestophandler() {
          var that=this;
          return function(s) {
            if(that.onresizestop instanceof Function) {
              that.onresizestop(this, s);
            }
            s.stopPropagation();
              console.debug({"stopping":this.dir})
      
            
          }
        });
        C.Def(function resizehandler() {
          var that=this;
          return function(state) {
            var orientation=this.orientations[this.dir];
            //var oldscale=that.imgscale*that.tempscale;
