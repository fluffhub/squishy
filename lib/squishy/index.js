...
var HASCAPTURESTACKTRACE = Error.captureStackTrace instanceof Function;
...
function mix(destination,source) {
  ...
  var my_descriptors= Object.getOwnPropertyNames(destination)
  for(var i=0;i<names.length;i++) {
    var name=names[i];
    my_desc = Object.getOwnPropertyDescriptor(destination,name)
      if(my_desc===undefined || my_desc&&my_desc.configurable) {
        Object.defineProperty(destination, name, descriptors[name])
      } 
  }
}
...
function Module() {
  if(this instanceof Module) {
    ...
    if(element) {// instanceof Element && Object.keys(element).indexOf("Module")<0) {
      ...
      var dir=ps.slice(0,-1);
      var site = [parser.protocol+'/', parser.hostname];
      element.root=site.concat(dir).join('/');
      ...
      if(vn=='index') element.name=ps.slice(-2)[0];
        ...
        if(ft=='js') element.name=vn;
          ...
          element.name=fn;
          element.root=site.concat(ps).join('/');
        }
      }
      ...
      if(M) {
        if(M.def===undefined) M.def=callback;
...
Object.defineProperty(Module,"Template",{value:{},writable:false, configurable:false});
Object.defineProperty(Module,"addTemplate",{configurable:false,value:function(tpl) {
  if(tpl instanceof Function) {
    if(typeof tpl.name === "string" && tpl.name!="")  {
      Object.defineProperty(Module.Template,tpl.name,{value:tpl, enumerable:true,configurable:false});
    } else {
      throw new Error("Tried to add function template without name.");
    }
  } else {
    throw new Error("Tried to add non-function template.");
  }
}});
Module.addTemplate(function Def() {
    var result;
    if(typeof(arguments[0])=='function') {
      //include the module's path in the function
      arguments[0].src=this.filename+'#'+arguments[0].name; 
      Object.defineProperty(this,arguments[0].name,{value:arguments[0]});
      result=arguments[0];
    } else {
  ...
  Module.addTemplate(function Index() {
  ...
  Module.addTemplate(function Import(paths,callback2) {
  ...
  Module.addTemplate(function Templates(file) {
  ...
  function identify(v) {
    if ((v.element && v.element instanceof Element)  || v instanceof Element) {
      var uid;
      if (v instanceof Element) {
        Import("squishy/DOM",function(DOM) {
          if(v.Tag) v=v.Tag;
          else v = new DOM.Tag(v);
        });
      }

      return v.element.uid();
  } else {
    return null;
  }
}
Element.prototype.uid=function element_uid() {
  var uid;
  var v=this;
  if (v.hasAttribute("data-id")) {
    uid = v.getAttribute("data-id");
  } else{
 ...
 Object.defineProperty(Module,"__setattr__",{value:function __setattr__(k,v) {
    var t=this;
    
      if(t.element instanceof Element) {
        if(v!==null && v!==undefined && ((v.element && v.element instanceof Element && v.element.parentElement !== null) || (v instanceof Element && v.parentElement !== null))) {

          var uid=identify(v);

          if(uid) t.element.setAttribute("data-e-"+k,uid)
    } else if (v instanceof Function) {
      t.element.setAttribute("data-f-"+k,btoa(v.toString()));
    } else {
      try {
        t.element.setAttribute("data-j-"+k,
        JSON.stringify(v, function domObjectReplacer(k,v) {
          if(v!==null && v!==undefined && ((v.element && v.element instanceof Element && v.element.parentElement !== null) || (v instanceof Element && v.parentElement !== null))) {
            var uid=identify(v);
            if(uid) return { __element__:uid };
          
          }
          return v;
        }));
      } catch(e) {
        console.log({"Setter failed to parse":v})
      }
    }
  }
  
  }});
  Object.defineProperty(Module,"__construct__",{value:function __construct__(po) {
    po.element.Tag=po;
  }});
  
  Object.defineProperty(Module,"__getter__",{value:function __getter__(t, k) {
    if(t[k]!==undefined) {
      return Reflect.get(t,k)
    } else {
      // try to get a data-*-property value from this.element
      if(t.element) {
        if(t.element.hasAttribute("data-j-"+k)) {
          return JSON.parse(te.element.getAttribute("data-j-"+k))
        }

      }
    }
  }});
  Object.defineProperty(Module, "__setter__",{value:function __setter__(t,k,v) {
    var result, target;

    if (k!=="element" && (!(k in t) || Object.keys(t).indexOf(k)>=0)) {
      if(v===null || v===undefined) target=v 
     
      else if(v.__watched__ || v.__proxied__) target=v;
      else target=Watch(v,function(obj) {
        Module.__setattr__.call(t,k,v)
      });
      result=Reflect.set(t,k,target);
      Module.__setattr__.call(t,k,v)
    ...
    if(typeof(path)=="string") {
      ...
      if(ft in Import.types) {
        ...
        if(script) {
          ...
          else script.Module=new Proxy(new window.Module(path), {
            construct:Module.__construct__,
            get:Module.__getter__,
            set:Module.__setter__});
...
function Watch(i_obj,fun, config) {
  var depth=2;
  ...
  var setter, getter;
  if(config.setter instanceof Function) setter=config.setter;
  if(config.getter instanceof Function) getter=config.getter;
  function _watch(obj, depth) {
    if(typeof obj == "object" && obj instanceof Object && obj !== document && obj !== window) {
      var keys = Object.keys(obj);
      for (var i=0;i<keys.length;i++) {
        var key = keys[i];
        if(depth>0)
        obj[key]=_watch(obj[key], depth-1)
      }
      if(obj.__watched__) return obj;
      return new Proxy(obj, {
        get: function(t,k) {
          if (k==="__self__") { 
            if(t.__proxied__) return t.__self__; else return t;    
          }
          if(k==="__watched__") return true;
          if(k==="__proxied__") return true;
          else if(getter instanceof Function) return getter(t,k);
          else return Reflect.get(t,k);
        },
        set: function(t,k,v) {
          var ret;
          if(setter instanceof Function) ret=setter(t,k,v);
          else ret=Reflect.set(t,k,_watch(v));
          fun(i_obj);
          return ret;
        }
      });
...
function Class(def,fun) {
  ...
  function Super(fun,cls) {
    ...
    if(cls.proxyfunctions && cls.proxyfunctions.proxied===true) {
      extend(fun.proxyfunctions, cls.proxyfunctions);
    }
  ...
  function Mixin(fun, cls) {
    ...
    if(typeof cls == 'object' && cls.constructor == Object) {
      //mix(fun.prototype,cls);
      extend(fun.prototype,cls);
    } else if(typeof cls=='function') {
      extend(fun,cls);
      mix(fun.prototype, cls.prototype);
    }
    if(!(fun.prototype.supers.indexOf(cls))) fun.prototype.supers.push(cls);
  }
  if(fun.Template) {} else Object.defineProperty(fun,'Template',{value:{}});
  if(fun.proxyfunctions) {} else Object.defineProperty(fun,'proxyfunctions',{value:{
    proxied: false,
    construct: function(t,a) { return Reflect.construct(t, a) },
    getter: function(t,k) { 
      if (k==="__proxied__") return true;
      if (k==="__self__") { 
        if(t.__proxied__) return t.__self__; else return t;    
    ...
    setter: function(t,k,v) { return Reflect.set(t,k,v) },
  }});
  extend(fun.Template,{
    ...
    Init:function Init(con) {
      ...
      Object.defineProperty(con,'proxyfunctions',{value:fun.proxyfunctions});
      fun=con;
    ...
    Def:function Def(f) {
      ...
      if(typeof(arguments[0])=='function') {
        ...
        Object.defineProperty(
          fun.prototype,arguments[0].name,
          {value:arguments[0],configurable:true}
        );
        result=arguments[0];
      } else {
        if(arguments.length==2) {
          Object.defineProperty(
            ...
            {value:arguments[1],writable:true}
          );
          result=arguments[1];
        }
      }
    ...
    Get:function __getter__(callback) {
      fun.proxyfunctions.getter=function(t,k) {
        if (k==="__self__") { 
          if(t.__proxied__) return t.__self__; else return t;    
        }
        if(k==="__proxied__") return true;
      ...
      fun.proxyfunctions.proxied=true;
    },
    Set:function __setter__(callback) {
      fun.proxyfunctions.setter=callback;
      fun.proxyfunctions.proxied=true;
    },
    Construct:function __construct__(callback) {
      if(callback instanceof Function) {
        fun.proxyfunctions.construct=callback;
        fun.proxyfunctions.proxied=true;
      } else {
        throw new Error("Non constructor function used: "+callback.toString());
      }
    }
  });
  ...
  Object.defineProperty(fun,'Mixin',{value:function(cls) { Mixin(fun, cls); } });
  Object.defineProperty(fun.prototype,'Mixin',{writable:true,enumerable:false });
  
  fun.prototype.Mixin=function(cls) {
    if(typeof cls=='function') {
      mix(this,cls.prototype);
      cls.apply(this, Array.prototype.slice(arguments,1));
  ...
  fun.isClass=true;
  if(fun.proxyfunctions.proxied) {
    var po = new Proxy(fun, {
      construct (t,a) {
        var T = Object.create(t.prototype);//new t(...a);
        var ProxyObject = new Proxy(T, {
          get: function getter(t,k) {
            if (k==="__self__") { 
              if(t.__proxied__) return t.__self__; else return t;    
            }
            if (k==="__proxied__") return true;
            return fun.proxyfunctions.getter(t,k)
          },
          set: fun.proxyfunctions.setter,
          getPrototypeOf(t, o) {
            return fun.prototype;
          },
        });
        t.apply(ProxyObject,a)
        fun.proxyfunctions.construct(ProxyObject);
        return ProxyObject;
      },
      apply: function (t,that, args) {
        return fun.apply(that, args);
      },


    });
    po.prototype=fun.prototype;
    return po;
  }
...
Module.addTemplate(function Class(def) {
...
Module(function M() {
  ...
  M.Def("Class",Class);
  if (document && !!window) {
    Import("squishy/DOM", "squishy/live", function(DOM, live) {
      var frame = new DOM.Frame({win:window, doc:document});
      frame.query("*");
    });
  }
});
