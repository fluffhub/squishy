Module(function M() {
  var Style=M.Class(function C() {
    ...
    C.Def(function extract(recursive,unique, item,output,sheets) {
      ...
      for (var k=0;k< b.children.length;k++) {
          ...
          for (var r=0;r< rules.length;r++) {
            if(c.matches instanceof Function)  {
              if(c.matches(rules[r].selectorText)) {
                ...
                if(unique) {
                  var els=document.querySelectorAll(":not(."+b.className+",."+b.className+")");
                  var failed=false;
                  for(var i=0;i<els.length;i++) {
                    var el=els[i];
                    if(el.matches instanceof Function) {
        ...
        if(c.children!==undefined && c.children.length>0&&recursive)
          extract(recursive,c,output,sheets);
