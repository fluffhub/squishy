Module(function M() {
  var Style=M.Class(function C() {
    ...
    C.Def(function extract(recursive,item,output,sheets) {
    ...
    C.Def(function clear(recursive, item, sheets) {
      var b=this.element;
      if(item instanceof Element) b=item;
      var obj=this.extract(recursive,item);
      Object.keys(obj).forEach(function(shn) {
        var sh=obj[shn];
        Object.keys(sh).forEach(function(rn) {
          var r=sh[rn];
          r.parentStyleSheet.removeRule(rn);
        });
      });
    });
