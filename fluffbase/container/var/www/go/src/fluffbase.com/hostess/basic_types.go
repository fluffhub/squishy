...
func ApplyTemplate(path string, context Env) ([]byte, error) {
	...
	if err==nil {
		...
		if bytes.Compare(data[0:2], []byte("#!"))==0 {
			...
			err := filepath.Walk(template, func(path string, info os.FileInfo, err error) error {
				...
				if path!=template {
					...
					if mode & os.ModeSymlink != 0 {
						...
						fmt.Printf("Copying from template %s to %s;\n",path, targetpath)
						cmds=append(cmds, Exec{Cmd:fmt.Sprintf("su {name} -c 'cp %s %s'", path, targetpath)})							
						if ext=="cgi" {
							cmds = append(cmds, Exec{Cmd:fmt.Sprintf("chmod 755 %s", targetpath)})
						}
					} else if mode & 0111 != 0 {
...
func (p Package) Initialize(overwrite bool,  name string, src string) error {
	...
	cmds.Push(
		...
		Exec{Cmd:"chown -R {name}:{name} /home/{name}", Sudo:true},
	)
	/*cmds.Push(
...
//	cmds.Push(p.LoadTemplate(path, Env{})...)
	...
	cmds.Push(Exec{Cmd:"chown -R {name}:{name} /home/{name}", Sudo:true})
	if p.GitRepo {
		cmds.Push(Exec{Sudo:true, Cmd:`su {name} -c '
			ln -s /usr/lib/git-core/git-http-backend /home/{name}/public/git;
			git config --global user.email pkg_{name}@fluffbase.com;
...
func init() {
	...
	addHostess("wkr-lcl", Package{Template:"wkr", Type:"wkr", GitRepo:true, Membrane:true})
}